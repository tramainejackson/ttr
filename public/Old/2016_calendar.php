	...
	foreach($calendar as $month) {
		...
		while ( $day <= $totalDays ) {  
			...
			if(in_array($newDate, $gameDates)) {
				$getIndGame = NBA_Calendar::get_games($year."-".$monthNum."-".$day);
				if($getIndGame[0]->home_team == "Sixers") {
					echo "<td class='weekDayContent homeGame'><span class='weekDayNum'>".$day."</span><span class='nbaMatchup'>".$getIndGame[0]->away_team." vs ".$getIndGame[0]->home_team."</span><span class='gameTime'>".$getIndGame[0]->game_time."</span></td>";
				} else {					
					echo "<td class='weekDayContent awayGame'><span class='weekDayNum'>".$day."</span><span class='nbaMatchup'>".$getIndGame[0]->away_team." vs ".$getIndGame[0]->home_team."</span><span class='gameTime'>".$getIndGame[0]->game_time."</span></td>";
				}
			} else {
