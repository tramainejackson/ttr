	...
	<div class="container-fluid bgrd3">
		...
		<div class="row">
			...
			<div class="col-12 col-lg-6">
				...
				{!! Form::open(['action' => ['Leagues\LeagueScheduleController@add_week', 'season' => $showSeason->id, 'year' => $showSeason->year], 'name' =>'new_week_form', 'method' => 'POST']) !!}
									...
									<div class="my-2">
										...
										@if($showSeason->courts()->count() > 0)
											<div class="col-auto" id="">
												<select class="mdb-select md-form" name="game_court[]" searchable="Search here..">
													<option value="" disabled selected>Choose Court Location</option>
													@foreach($showSeason->courts as $court)
														<option value="{{ $court->id }}">{{ $court->court_description }}</option>
													@endforeach
												</select>

												<label class="mdb-main-label" for="game_court">Choose Location</label>
											</div>
										@endif
									</div>
								</div>
